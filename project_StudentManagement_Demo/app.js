...
app.get('/add'          , mainctrl.showAdd);//呈递添加学生页面

app.post    ('/student'         , mainctrl.doAddStudent); // Ajax接口提交表单，保存到数据库
app.get     ('/student'			, mainctrl.getAllStudent);	//Ajax接口得到所有学生

app.propfind('/student/:sid'    , mainctrl.check); //Ajax接口检查用户名是否被占用
app.get     ('/student/:sid'	, mainctrl.showUpdate);		//呈递更改学生页面
app.post    ('/student/:sid'	, mainctrl.updateStudent);	//Ajax接口，更改学生
app.delete  ('/student/:sid'	, mainctrl.deleteStudent);  //Ajax接口，删除学生

// app.delete('/student/add',mainctrl.doAdd)
